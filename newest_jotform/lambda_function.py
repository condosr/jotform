...
def lambda_handler(event, context):
    ...
    if existing_form == True:
        ...
        #This call below references teh glue api utilizing the boto3 library. It creates a db in glue, creates a crawler in glue, then runs the crawler
        print('glue_integration_db_make')
        glue_integration.db_make(formID)
        
        print('glue_integration_crawler_make')
        glue_integration.crawler_make(formID)
        
        print('glue_integration_start_crawler')
        glue_integration.start_crawler(formID)
